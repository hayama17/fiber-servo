 ...
 * an idempotent re-apply genuinely produces no ops rather than that being an
 * assumption about a stub.
 */
...
describe('explicit apply', () => {
  it('keeps edits pending, refreshes local imports, reports no-ops/errors, and tears down the session', async () => {
    ...
    // A plain ReplicaSet, so scaling is just "create the newly-desired
    // containers": no template-generation digest in the names to work
    // around in assertions.
    const app = `import { Container, ReplicaSet } from 'fiber-servo';
    ...
    try {
      ...
      expect(up.output()).not.toContain('create web-1');
      const apply = cli(['apply', file]);
      expect(await apply.exited).toBe(0);
      expect(apply.output()).toContain('create web-1');
      expect(apply.output()).toContain('create web-2');
      expect(apply.output()).not.toContain('remove web-0');
      // Re-applying the same file is a true no-op: the plan compares against
      // the spec digest each container was created from, not against a diff
      ...
      expect(duplicate.output()).not.toContain('create ');
      ...
      expect(up.output()).not.toContain('remove ');

      await writeFile(config, 'export const replicas = 1;');
      const result = await requestApply(file);
      expect(result.ok).toBe(true);
      expect(result.ops.filter((op) => op.startsWith('remove '))).toHaveLength(2);
      await up.stop();
  ...
  it('serializes apply and shutdown, and returns runtime errors to the caller', async () => {
    ...
    const session = createSession(
      {
        // A minimal Runtime that never actually records anything it
        // "applies": every reconcile pass reads desired vs *observed* state
        // fresh (serve.ts's `pass()`), so a container that was never
        // observed is desired but missing on every single pass, not just
        // the first. That is what lets this runtime stay this small and
        // still prove the session queues applies and surfaces each one's
        // own errors.
        runtime: (ctx) => ({
          async apply() {
            events.push('apply');
          ...
          async inspect() {
            return { containers: new Map(), revision: 0 };
          },
          subscribe() {
            return () => {};
          },
      ...
      async () => {
        ...
        return <Container name="web" image="nginx" />;
      },
    );
    ...
    // The first apply was never observed as succeeding, so the second apply
    // finds the same container still missing and tries again — and fails
    // again.
    expect((await second).errors).toEqual(['runtime refused create']);
    await stopping;
    expect(loads).toBe(2);
    // Two failed applies, then an unconditional `down()` on shutdown —
    // `Runtime.down` removes the whole application regardless of what was
    // ever successfully observed, unlike the old op-based teardown, which
    // only issued a `remove-pod` for a Pod it had actually seen exist.
    expect(events).toEqual(['apply', 'apply', 'down']);
    await session.stop();
    expect(events).toEqual(['apply', 'apply', 'down']);
  });
